...
async function debug(taskArguments, hre) {
  ...
  const trillestProxy = await hre.ethers.getContract("TRILLESTProxy");
  ...
  const trillest = await hre.ethers.getContractAt(
    "TRILLEST",
    trillestProxy.address
  );
  const cTrillest = await hre.ethers.getContract("TRILLEST");
  ...
  console.log("\nContract addresses");
  console.log("====================");
  console.log(`TRILLEST proxy:              ${trillestProxy.address}`);
  console.log(
    `TRILLEST impl:               ${await trillestProxy.implementation()}`
  );
  console.log(`TRILLEST:                    ${cTrillest.address}`);
  ...
  const trillestGovernorAddr = await trillest.governor();
  ...
  console.log("\nGovernor addresses");
  console.log("====================");
  console.log("TRILLEST:              ", trillestGovernorAddr);
  ...
  // TRILLEST
  //
  const name = await trillest.name();
  const decimals = await trillest.decimals();
  const symbol = await trillest.symbol();
  const totalSupply = await trillest.totalSupply();
  const vaultAddress = await trillest.vaultAddress();
  const nonRebasingSupply = await trillest.nonRebasingSupply();
  const rebasingSupply = totalSupply.sub(nonRebasingSupply);
  const rebasingCreditsPerToken = await trillest.rebasingCreditsPerToken();
  const rebasingCredits = await trillest.rebasingCredits();
