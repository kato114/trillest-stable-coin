...
// By default, mint 1k worth of TRILLEST for each test account.
const defaultMintAmount = 1000;

// By default, redeem 1k worth of TRILLEST for each test account.
...
/**
 * Mints TRILLEST using USDT on local or fork.
 */
async function mint(taskArguments, hre) {
  ...
  for (let i = accountIndex; i < accountIndex + numAccounts; i++) {
    ...
    console.log(
      `Minting ${mintAmount} TRILLEST for account ${i} at address ${address}`
    );
    ...
    const trillestBalance = await trillest.balanceOf(address);
    console.log(
      "New TRILLEST balance=",
      hre.ethers.utils.formatUnits(trillestBalance, 18)
    );
  }
}

/**
 * Redeems TRILLEST on fork for specific account
 */
async function redeemFor(taskArguments, hre) {
  ...
  console.log(`Redeeming ${redeemAmount} TRILLEST for address ${address}`);

  // Show the current balances.
  let trillestBalance = await trillest.balanceOf(address);
  ...
  console.log("TRILLEST balance=", trillestUnitsFormat(trillestBalance, 18));
  ...
  await vault
    .connect(signer)
    .redeem(
      trillestUnits(redeemAmount),
      trillestUnits((redeemAmountInt - redeemAmountInt * 0.05).toString()),
      { gasLimit: 2500000 }
    );

  // Show the new balances.
  trillestBalance = await trillest.balanceOf(address);
  daiBalance = await dai.balanceOf(address);
  usdcBalance = await usdc.balanceOf(address);
  usdtBalance = await usdt.balanceOf(address);
  console.log(
    "New TRILLEST balance=",
    trillestUnitsFormat(trillestBalance, 18)
  );
...
/**
 * Redeems TRILLEST on local or fork.
 */
async function redeem(taskArguments, hre) {
  ...
  for (let i = accountIndex; i < accountIndex + numAccounts; i++) {
    ...
    const address = signer.address;
    console.log(
      `Redeeming ${redeemAmount} TRILLEST for account ${i} at address ${address}`
    );
    ...
    await vault
      .connect(signer)
      .redeem(trillestUnits(redeemAmount), 0, { gasLimit: 2000000 });
...
// Sends TRILLEST to a destination address.
async function transfer(taskArguments) {
  ...
  console.log("\nTRILLEST balances prior transfer");
  console.log(
    `${signer.address}: ${trillestUnitsFormat(
      await trillest.balanceOf(signer.address)
    )} TRILLEST`
  );
  console.log(
    `${to}: ${trillestUnitsFormat(await trillest.balanceOf(to))} TRILLEST`
  );

  // Send TRILLEST.
  console.log(
    `\nTransferring ${amount} TRILLEST from ${signer.address} to ${to}...`
  );
  await trillest.connect(signer).transfer(to, trillestUnits(amount));

  // Print balances after to the transfer
  console.log("\nTRILLEST balances after transfer");
  console.log(
    `${signer.address}: ${trillestUnitsFormat(
      await trillest.balanceOf(signer.address)
    )} TRILLEST`
