...
describe("Vault with Compound strategy", function () {
  ...
  it("Governor can call setPTokenAddress", async () => {
    const { dai, trillest, matt, compoundStrategy } = await loadFixture(
      compoundVaultFixture
    );
    await expect(
      compoundStrategy
        .connect(matt)
        .setPTokenAddress(trillest.address, dai.address)
  ...
  it("Should correctly handle a deposit of USDC (6 decimals)", async function () {
    const { anna, trillest, usdc, vault } = await loadFixture(
      compoundVaultFixture
    );
  ...
  it("Should allow withdrawals", async () => {
    const { anna, compoundStrategy, trillest, usdc, vault, governor } =
      await loadFixture(compoundVaultFixture);
    ...
    await trillest.connect(anna).approve(vault.address, trillestUnits("40.0"));
    await vault.connect(anna).redeem(trillestUnits("40.0"), 0);

    await expect(anna).has.an.approxBalanceOf("10", trillest);
  ...
  it("Should calculate the balance correctly with DAI in strategy", async () => {
    ...
    // Josh had 1000 DAI but used 100 DAI to mint TRILLEST in the fixture
  ...
  it("Should not alter balances after an asset price change", async () => {
    let { trillest, vault, matt, usdc, dai } = await loadFixture(
      compoundVaultFixture
    );
    ...
    // 200 TRILLEST was already minted in the fixture, 100 each for Matt and Josh
    await expectApproxSupply(trillest, trillestUnits("600.0"));
    // 100 + 200 + 200
    await expect(matt).has.an.approxBalanceOf("500", trillest, "Initial");
  ...
  it("Should handle non-standard token deposits", async () => {
    let { trillest, vault, matt, nonStandardToken, governor } =
      await loadFixture(compoundVaultFixture);
    ...
    try {
      ...
      await expectApproxSupply(trillest, trillestUnits("200.0"));
      await expect(matt).has.an.approxBalanceOf("100", trillest);
    ...
    await expectApproxSupply(trillest, trillestUnits("300.0"));
    await expect(matt).has.an.approxBalanceOf("200", trillest, "Initial");
    await vault.rebase();
    await expect(matt).has.an.approxBalanceOf(
      "200",
      trillest,
  ...
  it("Should allocate correctly with DAI when Vault buffer is 1e17 (10%)", async () => {
    ...
    await expect(
      await compoundStrategy.checkBalance(dai.address)
    ).to.approxEqual(trillestUnits("180"));
    // Remaining 20 should be in Vault
    await expect(await vault.totalValue()).to.approxEqual(trillestUnits("200"));
  });
  ...
  it("Should allow transfer of arbitrary token by Governor", async () => {
    const { vault, compoundStrategy, trillest, usdc, matt, governor } =
      await loadFixture(compoundVaultFixture);
    // Matt deposits USDC, 6 decimals
    await usdc.connect(matt).approve(vault.address, usdcUnits("8.0"));
    await vault.connect(matt).mint(usdc.address, usdcUnits("8.0"), 0);
    // Matt sends his TRILLEST directly to Strategy
    await trillest
      .connect(matt)
      .transfer(compoundStrategy.address, trillestUnits("8.0"));
  ...
  it("Should not allow transfer of arbitrary token by non-Governor", async () => {
    const { compoundStrategy, trillest, matt } = await loadFixture(
      compoundVaultFixture
    );
    // Naughty Matt
    await expect(
...
describe("Vault auto allocation", async () => {
  ...
  const setThreshold = async (amount) => {
    ...
    await vault
      .connect(governor)
      .setAutoAllocateThreshold(trillestUnits(amount));
  };
  ...
  it("Alloc with both threshhold and buffer", async () => {
    ...
    await vault.allocate();
    await vault.connect(governor).setVaultBuffer(utils.parseUnits("1", 17));
    await vault.connect(governor).setAutoAllocateThreshold(trillestUnits("3"));
