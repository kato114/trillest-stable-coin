...
describe("Vault mock with rebase", async () => {
  it("Should increase users balance on rebase after increased Vault value", async () => {
    const { mockVault, matt, trillest, josh } = await loadFixture(
      mockVaultFixture
    );
    // Total TRILLEST supply is 200, mock an increase
    await mockVault.setTotalValue(utils.parseUnits("220", 18));
    await mockVault.rebase();
    await expect(matt).has.an.approxBalanceOf("110.00", trillest);
    await expect(josh).has.an.approxBalanceOf("110.00", trillest);
  });
  ...
  it("Should not decrease users balance on rebase after decreased Vault value", async () => {
    ...
    // Total TRILLEST supply is 200, mock a decrease
