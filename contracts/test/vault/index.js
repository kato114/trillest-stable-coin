...
describe("Vault", function () {
  ...
  it("Should support an asset", async () => {
    const { vault, oracleRouter, trillest, governor } = await loadFixture(
      defaultFixture
    );
    const oracleAddresses = await getOracleAddresses(hre.deployments);
    const origAssetCount = await vault.connect(governor).getAssetCount();
    expect(await vault.isSupportedAsset(trillest.address)).to.be.false;
    await oracleRouter.setFeed(
      trillest.address,
      oracleAddresses.chainlink.DAI_USD
    );
    await expect(
      vault.connect(governor).supportAsset(trillest.address)
    ...
    expect(await vault["checkBalance(address)"](trillest.address)).to.equal(0);
    expect(await vault.isSupportedAsset(trillest.address)).to.be.true;
  });
  ...
  it("Should correctly ratio deposited currencies of differing decimals", async function () {
    const { trillest, vault, usdc, dai, matt } = await loadFixture(
      defaultFixture
    );
    ...
    await expect(matt).has.a.balanceOf("102.00", trillest);
    ...
    await expect(matt).has.a.balanceOf("106.00", trillest);
  });

  it("Should correctly handle a deposit of DAI (18 decimals)", async function () {
    const { trillest, vault, dai, anna } = await loadFixture(defaultFixture);
    await expect(anna).has.a.balanceOf("0.00", trillest);
    // We limit to paying to $1 TRILLEST for for one stable coin,
    ...
    await expect(anna).has.a.balanceOf("3.00", trillest);
  });
  ...
  it("Should correctly handle a deposit of USDC (6 decimals)", async function () {
    ...
    await expect(anna).has.a.balanceOf("48.00", trillest);
  });

  it("Should correctly handle a deposit failure of Non-Standard ERC20 Token", async function () {
    const { trillest, vault, anna, nonStandardToken, governor } =
    ...
    // fails silently, so Anna's TRILLEST balance should be zero.
    try {
      await vault
        .connect(anna)
  ...
  it("Should allow transfer of arbitrary token by Governor", async () => {
    const { vault, trillest, usdc, matt, governor } = await loadFixture(
      defaultFixture
    );
    // Matt deposits USDC, 6 decimals
    await usdc.connect(matt).approve(vault.address, usdcUnits("8.0"));
    ...
    await trillest.connect(matt).transfer(vault.address, trillestUnits("8.0"));
  ...
  it("Should not allow transfer of supported token by governor", async () => {
    ...
    await expect(
      vault.connect(governor).transferToken(usdc.address, trillestUnits("8.0"))
  ...
  it("Should allow Governor to add Strategy", async () => {
    ...
    // Pretend TRILLEST is a strategy and add its address
    await vault.connect(governor).approveStrategy(trillest.address);
  });

  it("Should revert when removing a Strategy that has not been added", async () => {
    const { vault, governor, trillest } = await loadFixture(defaultFixture);
    // Pretend TRILLEST is a strategy and remove its address
    await expect(
      vault.connect(governor).removeStrategy(trillest.address)
    ).to.be.revertedWith("Strategy not approved");
  });

  it("Should correctly handle a mint with auto rebase", async function () {
    const { trillest, vault, usdc, matt, anna } = await loadFixture(
      defaultFixture
    );
    await expect(anna).has.a.balanceOf("0.00", trillest);
    await expect(matt).has.a.balanceOf("100.00", trillest);
    ...
    await expect(anna).has.a.balanceOf("5000.00", trillest);
    await expect(matt).has.a.balanceOf("100.00", trillest);
  });

  it("Should revert mint if minMintAmount check fails", async () => {
    const { vault, matt, trillest, dai, usdt } = await loadFixture(
      defaultFixture
    );
  ...
  it("Should allow governor to change rebase threshold", async () => {
    ...
    await vault.connect(governor).setRebaseThreshold(trillestUnits("400"));
  });

  it("Should not allow non-governor to change rebase threshold", async () => {
    const { vault } = await loadFixture(defaultFixture);
    expect(vault.setRebaseThreshold(trillestUnits("400"))).to.be.revertedWith(
      "Caller is not the Governor"
    );
  });
