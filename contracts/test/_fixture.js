...
async function defaultFixture() {
  ...
  const liquidityRewardTRILLEST_USDT = await ethers.getContractAt(
    "LiquidityReward",
    (
      await ethers.getContract("LiquidityRewardTRILLEST_USDTProxy")
    ).address
  );
  ...
  if (isFork) {
    ...
    uniswapPairTRILLEST_USDT = await ethers.getContract(
      "MockUniswapPairTRILLEST_USDT"
    );
    ...
    await mockNonRebasing.setTRILLEST(trillest.address);
    mockNonRebasingTwo = await ethers.getContract("MockNonRebasingTwo");
    await mockNonRebasingTwo.setTRILLEST(trillest.address);
  ...
  // Matt and Josh each have $100 TRILLEST
...
async function rebornFixture() {
  ...
  const rebornAttack = async (shouldAttack = true, targetMethod = null) => {
    ...
    await sanctum.setTRILLESTAddress(fixture.trillest.address);
    await sanctum.deploy(12345, deployCode);
  };
