...
describe("3Pool Strategy", function () {
  ...
  describe("Mint", function () {
    ...
    it("Should stake USDC in Curve gauge via 3pool", async function () {
      ...
      await expectApproxSupply(trillest, trillestUnits("50200"));
      await expect(anna).to.have.a.balanceOf("50000", trillest);
      await expect(threePoolGauge).has.an.approxBalanceOf(
        "50000",
        threePoolToken
      );
  ...
  describe("Utilities", function () {
    it("Should allow transfer of arbitrary token by Governor", async () => {
      ...
      await trillest
        .connect(anna)
        .transfer(threePoolStrategy.address, trillestUnits("8.0"));
