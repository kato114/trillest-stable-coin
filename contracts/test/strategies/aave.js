...
describe("Aave Strategy", function () {
  ...
  describe("Mint", function () {
    it("Should be able to mint DAI and it should show up in the Aave core", async function () {
      ...
      await expectApproxSupply(trillest, trillestUnits("30200"));
      // should allocate all of it to strategy
      await expect(aaveStrategy).has.an.approxBalanceOf("30200", adai);
      await expect(anna).to.have.a.balanceOf("30000", trillest);
    ...
    it("Should not send USDC to Aave strategy", async function () {
      await emptyVault();
      // should be all empty
      await expectApproxSupply(trillest, trillestUnits("0"));
      await mint("30000.00", usdc);
      await expectApproxSupply(trillest, trillestUnits("30000"));
      await expect(aaveStrategy).has.an.approxBalanceOf("0", dai);
      await vault.connect(anna).redeem(trillestUnits("30000.00"), 0);
    });

    it("Should be able to mint and redeem DAI", async function () {
      await expectApproxSupply(trillest, trillestUnits("200"));
      await mint("30000.00", dai);
      await vault.connect(anna).redeem(trillestUnits("20000"), 0);
      await expectApproxSupply(trillest, trillestUnits("10200"));
      // Anna started with 1000 DAI
      await expect(anna).to.have.a.balanceOf("21000", dai);
      await expect(anna).to.have.a.balanceOf("10000", trillest);
    });
    ...
    it("Should be able to redeem and return assets after multiple mints", async function () {
      ...
      await mint("30000.00", usdc);
      await mint("30000.00", dai);
      await vault.connect(anna).redeem(trillestUnits("60000.00"), 0);
    ...
    it("Should allow transfer of arbitrary token by Governor", async () => {
      ...
      // Anna sends her TRILLEST directly to Strategy
      await trillest
        .connect(anna)
        .transfer(aaveStrategy.address, trillestUnits("8.0"));
