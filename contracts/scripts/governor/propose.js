...
// Transfer governance of the TRILLEST contract from old to new governor.
// IMPORTANT: must be executed against the old governor.
async function proposeTrillestNewGovernorArgs() {
  const { governorAddr } = await getNamedAccounts();
  const cTRILLESTProxy = await ethers.getContract("TRILLESTProxy");
  const cTRILLEST = await ethers.getContractAt(
    "TRILLEST",
    cTRILLESTProxy.address
  );

  const description = "TRILLEST governance transfer";
  const args = await proposeArgs([
    {
      contract: cTRILLEST,
...
// - upgradeTo TRILLEST
async function proposeTrillestv2ResetArgs() {
  const dTRILLEST = await ethers.getContract("TRILLEST");
  const dTRILLESTReset = await ethers.getContract("TRILLESTReset");
  const cTRILLESTProxy = await ethers.getContract("TRILLESTProxy");
  ...
  const description = "TRILLEST Reset";
  const args = await proposeArgs([
    {
...
// Returns the argument to use for sending a proposal to upgrade TRILLEST.
...
// Returns the argument to use for sending a proposal to upgrade TRILLEST.
async function proposeUpgradeTrillestArgs() {
  const trillestProxy = await ethers.getContract("TRILLESTProxy");
  const trillest = await ethers.getContract("TRILLEST");

  const args = await proposeArgs([
    {
      contract: trillestProxy,
      signature: "upgradeTo(address)",
      args: [trillest.address],
    },
  ]);
  const description = "Upgrade TRILLEST";
  return { args, description };
}
...
async function main(config) {
  ...
  } else if (config.upgradeTrillest) {
    console.log("upgradeTrillest proposal");
    argsMethod = proposeUpgradeTrillestArgs;
  ...
  } else if (config.trillestNewGovernor) {
    console.log("TrillestNewGovernor");
    argsMethod = proposeTrillestNewGovernorArgs;
  } else if (config.trillestv2Reset) {
    console.log("Trillestv2Reset");
    argsMethod = proposeTrillestv2ResetArgs;
...
const config = {
  ...
  upgradeTrillest: args["--upgradeTrillest"],
  ...
  trillestNewGovernor: args["--trillestNewGovernor"],
  trillestv2Reset: args["--trillestv2Reset"],
