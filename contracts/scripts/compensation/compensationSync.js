...
async function verify(expectedAccounts, dataFileLocation) {
  ...
  for (const account of expectedAccounts) {
    ...
    const walletAmount = await trillestContract.balanceOf(account.address);
    const contractStateCorrect = actual.eq(expected);

    // Because of rebasing logic of TRILLEST the amounts are not going to be completely exact
...
async function compensationSync(compContract, dataFileLocation, doIt, signer) {
  ...
  trillestContract = await ethers.getContractAt(
    "TRILLEST",
...
async function main() {
  ...
  await getContract();
  console.log("Connecting to TRILLEST compensation contract", contract.address);
