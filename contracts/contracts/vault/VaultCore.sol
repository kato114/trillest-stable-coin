...
/**
 * @title TRILLEST Vault Contract
 * @notice The Vault contract stores assets. On a deposit, TRILLEST will be minted
           and sent to the depositor. On a withdrawal, TRILLEST will be burned and
...
contract VaultCore is VaultStorage {
    ...
    /**
     * @dev Deposit a supported asset and mint TRILLEST.
     * @param _asset Address of the asset being deposited
     * @param _amount Amount of the asset being deposited
     * @param _minimumTrillestAmount Minimum TRILLEST to mint
     */
    function mint(
        address _asset,
        uint256 _amount,
        uint256 _minimumTrillestAmount
        ...
        if (_minimumTrillestAmount > 0) {
            require(
                priceAdjustedDeposit >= _minimumTrillestAmount,
        ...
        // Mint matching TRILLEST
        sTRILL.mint(msg.sender, priceAdjustedDeposit);
    ...
    /**
     * @dev Withdraw a supported asset and burn TRILLEST.
     * @param _amount Amount of TRILLEST to burn
    ...
    function _redeem(uint256 _amount, uint256 _minimumUnitAmount) internal {
        ...
        // Check that TRILLEST is backed by enough assets
        uint256 _totalSupply = sTRILL.totalSupply();
        if (maxSupplyDiff > 0) {
            // Allow a max difference of maxSupplyDiff% between
            // backing assets value and TRILLEST total supply
        ...
        sTRILL.burn(msg.sender, _amount);
    ...
    /**
     * @notice Withdraw a supported asset and burn all TRILLEST.
     * @param _minimumUnitAmount Minimum stablecoin units to receive in return
     */
    function redeemAll(uint256 _minimumUnitAmount)
        ...
        nonReentrant
    {
        _redeem(sTRILL.balanceOf(msg.sender), _minimumUnitAmount);
    }
     ...
     *      strategies and update the supply of TRILLEST.
     */
    ...
    /**
     * @dev Calculate the total value of assets held by the Vault and all
     *      strategies and update the supply of TRILLEST, optionally sending a
     *      portion of the yield to the trustee.
     */
    function _rebase() internal whenNotRebasePaused {
        uint256 trillestSupply = sTRILL.totalSupply();
        if (trillestSupply == 0) {
            return;
        }
        ...
        address _trusteeAddress = trusteeAddress; // gas savings
        if (_trusteeAddress != address(0) && (vaultValue > trillestSupply)) {
            uint256 yield = vaultValue.sub(trillestSupply);
            uint256 fee = yield.mul(trusteeFeeBps).div(10000);
            require(yield > fee, "Fee must not be greater than yield");
            if (fee > 0) {
                sTRILL.mint(_trusteeAddress, fee);
            }
            emit YieldDistribution(_trusteeAddress, yield, fee);
        }

        // Only rachet TRILLEST supply upwards
        trillestSupply = sTRILL.totalSupply(); // Final check should use latest value
        if (vaultValue > trillestSupply) {
            sTRILL.changeSupply(vaultValue);
        }
    }
        ...
        returns (uint256[] memory outputs, uint256 totalBalance)
    {
        ...
        // totalOutputRatio.
        //
        // For example, withdrawing: 30 TRILLEST:
        ...
        for (uint256 i = 0; i < allAssets.length; i++) {
            ...
            // stablecoin per dollar of TRILLEST
            if (price < 1e18) {
                price = 1e18;
            }
