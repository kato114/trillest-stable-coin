...
contract Buyback is Governable {
    ...
    // Address of TRILLEST Vault
    address public immutable vaultAddr;

    // Swap from TRILLEST
    IERC20 immutable trillest;
    ...
    constructor(
        ...
        // Give approval to Uniswap router for TRILLEST, this is handled
        // by setUniswapAddr in the production contract
        IERC20(_trillest).safeApprove(uniswapAddr, 0);
        IERC20(_trillest).safeApprove(uniswapAddr, type(uint256).max);
    }

    /**
     * @dev Verifies that the caller is the TRILLEST Vault.
     */
    modifier onlyVault() {
    ...
    function setUniswapAddr(address _address) external onlyGovernor {
        ...
        // Give Uniswap unlimited TRILLEST allowance
        trillest.safeApprove(uniswapAddr, 0);
        trillest.safeApprove(uniswapAddr, type(uint256).max);
        emit UniswapUpdated(_address);
    }

    /**
     * @dev Execute a swap of OGN for TRILLEST via Uniswap or Uniswap compatible
     * protocol (e.g. Sushiswap)
     **/
    function swap() external onlyVault nonReentrant {
        uint256 sourceAmount = trillest.balanceOf(address(this));
        ...
        uint256 minExpected = expectedOgnPerTRILLEST(sourceAmount).mul(97).div(100);

        UniswapV3Router.ExactInputParams memory params = UniswapV3Router
            .ExactInputParams({
                path: abi.encodePacked(
                    trillest,
                    uint24(500), // Pool fee, trillest -> usdt
                    usdt,
    ...
    function expectedOgnPerTRILLEST(uint256 trillestAmount)
        public
        view
        returns (uint256)
    {
        return
            trillestAmount.mul(uint256(1e26)).div( // ognEth is 18 decimal. ethUsd is 8 decimal.
