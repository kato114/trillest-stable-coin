...
import "../token/TRILLEST.sol";
import "../interfaces/Tether.sol";
import { IERC20 } from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import { SafeERC20 } from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

// Contract to exchange usdt, usdc, dai from and to trillest.
//   - 1 to 1. No slippage
...
contract Flipper is Governable {
    ...
    constructor(
        ...
        require(address(_trillest) != address(0));
        require(address(_usdc) != address(0));
        require(address(_usdt) != address(0));
        dai = IERC20(_dai);
        trillest = TRILLEST(_trillest);
        usdc = IERC20(_usdc);
        usdt = Tether(_usdt);
    }
    ...
    /// @notice Purchase TRILLEST with Dai
    /// @param amount Amount of TRILLEST to purchase, in 18 fixed decimals.
    function buyTrillestWithDai(uint256 amount) external {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
        require(
            dai.transferFrom(msg.sender, address(this), amount),
            "DAI transfer failed"
        );
        require(trillest.transfer(msg.sender, amount), "TRILLEST transfer failed");
    }

    /// @notice Sell TRILLEST for Dai
    /// @param amount Amount of TRILLEST to sell, in 18 fixed decimals.
    function sellTrillestForDai(uint256 amount) external {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
        require(dai.transfer(msg.sender, amount), "DAI transfer failed");
        require(
            trillest.transferFrom(msg.sender, address(this), amount),
            "TRILLEST transfer failed"
        );
    }

    /// @notice Purchase TRILLEST with USDC
    /// @param amount Amount of TRILLEST to purchase, in 18 fixed decimals.
    function buyTrillestWithUsdc(uint256 amount) external {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
    ...
    /// @notice Sell TRILLEST for USDC
    /// @param amount Amount of TRILLEST to sell, in 18 fixed decimals.
    function sellTrillestForUsdc(uint256 amount) external {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
        require(
    ...
    /// @notice Purchase TRILLEST with USDT
    /// @param amount Amount of TRILLEST to purchase, in 18 fixed decimals.
    function buyTrillestWithUsdt(uint256 amount) external {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
        // Potential rounding error is an intentional trade off
    ...
    /// @notice Sell TRILLEST for USDT
    /// @param amount Amount of TRILLEST to sell, in 18 fixed decimals.
    function sellTrillestForUsdt(uint256 amount) external {
        require(amount <= MAXIMUM_PER_TRADE, "Amount too large");
        // USDT does not return a boolean and reverts,
        // so no need for a require.
    ...
    /// trillest needs to do less accounting and one less storage write.
    function rebaseOptIn() external onlyGovernor nonReentrant {
        trillest.rebaseOptIn();
    }
    ...
    function withdrawAll() external onlyGovernor nonReentrant {
        ...
        IERC20(trillest).safeTransfer(_governor(), trillest.balanceOf(address(this)));
