...
/**
 * Deploy the core contracts (Vault and TRILLEST).
 *
 */
const deployCore = async () => {
  ...
  // Proxies
  await deployWithConfirmation("TRILLESTProxy");
  ...
  await withConfirmation(
    cTRILLESTProxy["initialize(address,address,bytes)"](
      dTRILLEST.address,
      governorAddr,
      []
    )
  );
  log("Initialized TRILLESTProxy");
  ...
  await withConfirmation(
    cVault
      .connect(sGovernor)
      .initialize(cOracleRouter.address, cTRILLESTProxy.address)
  );
  ...
  await withConfirmation(
    cTRILLEST
      .connect(sGovernor)
      .initialize("Origin Dollar", "TRILLEST", cVaultProxy.address)
  );

  log("Initialized TRILLEST");
};
...
// create Uniswap V3 TRILLEST - USDT pool
...
const deployWTrillest = async () => {
  const { deployerAddr, governorAddr } = await getNamedAccounts();
  const sDeployer = await ethers.provider.getSigner(deployerAddr);
  const sGovernor = await ethers.provider.getSigner(governorAddr);
  const trillest = await ethers.getContract("TRILLESTProxy");
  const dWrappedTrillestImpl = await deployWithConfirmation("WrappedTrillest", [
    trillest.address,
    "Wrapped TRILLEST IMPL",
    "WTRILLEST IMPL",
  ]);
  const dWrappedTrillestProxy = await deployWithConfirmation(
    "WrappedTRILLESTProxy"
  );
  const wtrillestProxy = await ethers.getContract("WrappedTRILLESTProxy");
  const wtrillest = await ethers.getContractAt(
    "WrappedTrillest",
    wtrillestProxy.address
  );

  await wtrillestProxy
    .connect(sDeployer)
  ...
  await wtrillest.connect(sDeployer)["initialize()"]();
  await wtrillest.connect(sDeployer).transferGovernance(governorAddr);
  await wtrillest.connect(sGovernor).claimGovernance();
};
