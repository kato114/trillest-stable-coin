...
const deployName = "008_trillest_reset";
...
const deployVault = async () => {
  ...
  await withConfirmation(
    cVault
      .connect(sDeployer)
      .initialize(cMixOracle.address, cTRILLESTProxy.address, await getTxOpts())
  );
...
const upgradeAndResetTRILLEST = async () => {
  ...
  const propTransferGovDescription = "TRILLEST governance transfer";
  ...
  if (isMainnet) {
    ...
    await withConfirmation(
      cTRILLESTProxy
        .connect(sGovernor)
        .upgradeTo(dTRILLESTReset.address, await getTxOpts())
    );
    ...
    await withConfirmation(cTRILLESTReset.connect(sGovernor).reset());
    log("Called reset on TRILLEST");

    await withConfirmation(
      cTRILLESTProxy
        .connect(sGovernor)
        .upgradeTo(dTRILLEST.address, await getTxOpts())
    );
...
//
// - TRILLEST transferGovernance to new governor
// - TRILLEST claimGovernance
// - TRILLESTProxy upgradeTo TRILLESTReset
// - TRILLESTReset reset()
// - TRILLESTProxy upgradeTo TRILLEST
